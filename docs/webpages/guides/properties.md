...
ESL name properties are strings with restrictions, as they must pass 
them as identifiers when the generated ESL code is given to the ESL 
...
floating point literal values; Integers type for integer literals; and 
...
taken from a 'Source'. The value is to be (the value of) an appropriate 
...
result from a Function Call. This output port match with another port 
...
!!! note
	...
	which has no console for to allow text input. The generated code 
...
| Time delay		| delay		| Frequency of communication region calls for the segment - a multiple of communication interval (CINT).<br>Note: The segment should have its Simulation Parameter TSTART set to this value. |
...
code that is generated for the subprogram diagram. 
